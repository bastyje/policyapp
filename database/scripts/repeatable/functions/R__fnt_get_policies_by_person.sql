IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id('fntGetPoliciesByPerson') AND xtype IN ('FN', 'IF', 'TF'))
	DROP FUNCTION [dbo].[fntGetPoliciesByPerson]
GO


CREATE FUNCTION [dbo].[fntGetPoliciesByPerson] (@personId INT)
RETURNS TABLE
AS
RETURN (
    SELECT
